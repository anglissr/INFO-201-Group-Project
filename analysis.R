...
emission_plot <- function(year, type) {
  ...
  total_emissions_year <- ggplot(emissions_us, aes(x = Year,
                                                   y = !!as.name(type),
                                                   group = 1)) +
                          geom_line() +
                          labs(title = "CO2 Emissions in the US",
                               subtitle = "(1800 - 2014)",
                               caption = "Data from the Carbon Dioxide
...
electric_vehicle_sales_type <-
  as_data_frame(cbind(Year, electric_vehicle_sales_type))
electric_vehicle_sales_type$Year <-
...
electric_vehicle_sales_type$V4 <-
  as.numeric(as.character(electric_vehicle_sales_type$V4))
...
epa_data <- epa_data[-c(1, 2, 95 : 105, 34, 54, 72, 57, 56, 35,
                        74, 73, 55, 84, 80, 89, 71, 90), ]
...
colnames(epa_chart_data) <- c("Source", "Year",
...
stacked_emissions_chart <- function(sources) {
  ...
  epa_bar_chart <- ggplot(varied_data) +
    geom_col(
      mapping = aes(x = `Year`, y = `Emissions (Million Metric Tonnes)`
                    , fill = `Source`)
    )
